...
const mapGeoCode = ({
    ...
    const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${option}.json?access_token=${process.env.MAPBOX_KEY}&limit=1`;

    request({url, json:true}, (err,res)=>{
        if(err){ //Low level Error like no internet
            callback('Cant Connect to geo service');        
        } else if(res.body.features.length === 0) { //Request Error
...
const ipGeoCode = (callback)=>{
    ...
    request({url, json:true}, (err,res)=>{
        if(err){ //Low level Error like no internet
            callback('Cant Connect to geo service');        
        } else if(res.status === "fail") { //Request Error
            callback('Cant find location')      
        } else {
            const data = res.body;
            callback(undefined,{
